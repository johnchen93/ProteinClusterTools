...
def AnnotateClusters(clusters, levels, annot_table, column, cluster_key, annot_key,  numeric_func='mean'): # map_func=None, mapping:dict=None,
    '''
    ...
    #     clusters[cluster_key]=clusters[cluster_key].map(mapping)
    # if map_func is not None:
    #     clusters[cluster_key]=clusters[cluster_key].apply(lambda x: map_func(x))

    # method='mean' if pd.api.types.is_numeric_dtype(annot_table[column]) else 'counts'
    ...
    for level in levels:
        ...
        if method!='counts':
            ...
            annot=annot[column].value_counts()
...
import random
c_template='rgba({},{},{},{})'
def ColorAnnot(annot:dict, cmap='viridis', top_n=None, saturation=1, shuffle_colors_seed=None, vmin=None, vmax=None, cmap_is_categorical=True, binary_blend=False):
    ...
    if annot['method']!='counts':
        # color_map = AdjustColormapSaturation(mpl.colormaps.get_cmap(cmap), saturation)
        ...
        # Choose a color map
        cmap_resolution=100
        colors['colorscale'] = [[i/(cmap_resolution-1), c_template.format(*[int(c*255) for c in color_map(i/(cmap_resolution-1))])] for i in range(cmap_resolution)]
        ...
        for level in annot['levels']:
            ...
            # format rgba string
            mapped_colors=[c_template.format(*[int(x*255) for x in c]) for c in mapped_colors]
        ...
        all_level_df=pd.concat(all_levels)
        all_cats=all_level_df.groupby('value')['count'].sum().reset_index().sort_values('count', ascending=False)['value'].unique()
        # include=all_cats if top_n is None else all_cats[:top_n]
        ...
        colors['categories']={k:c_template.format(*[int(x*255) for x in v]) for k,v in c_mapping.items()}# if k in include}
        ...
        for level in annot['levels']:
            ...
            for group, subdata in data.groupby('id'):
                ...
                if total==0:
                    ...
                    g/=total
                    b/=total
                    blended[group]=c_template.format(int(r*255),int(g*255),int(b*255),255)
